...
function ProjectsList() {
  ...
  const fetchProjects = () => {
    fetch(`https://${process.env.REACT_APP_API_URL}/projects`)
  ...
  const editItem = (userInput) => {
    ...
    try {
      fetch(`https://${process.env.REACT_APP_API_URL}/projects/${_id}`, {
        method: 'PUT',
  ...
  const deleteItem = () => {
    ...
    try {
      fetch(`https://${process.env.REACT_APP_API_URL}/projects/${id}`, params).then(() => {
  ...
  return (
    <div className={styles.container}>
      ...
      <button onClick={() => setModalAddItemOpen(true)}>Add</button>
      <table>
        <thead>
          <tr>
            <th>ID</th>
